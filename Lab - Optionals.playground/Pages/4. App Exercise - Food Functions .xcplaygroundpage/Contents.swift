...
func checkMeal(_ mealName :String)-> Meal?{
    return meals[mealName]
}
if let meal = checkMeal( "Breakfast"){
 ...
 */
func checkSaved(_ meal: String) -> [String: Any]? {
    return UserDefaults.standard.dictionary(forKey: meal)
}
if let meal = checkSaved("Breakfast"){
    print("Meal found location \(meal)")
}else{
